...
(function () {
     ...
     */
    function create() {
        var doc = {}, attr, value, matches, nextFix, aspect;
    ...
    function discoverPublicFunctions() {
        ...
        // Discover anoymous functions assigned to a variable
    ...
    function extractSignatureDocumentation(functionName) {
        ...
        // Split it into lines
        var lines = signatureDoc.split("\r\n");
        if (lines.length == 1 && expected > 1) {
            lines = signatureDoc.split("\n");
        }
        var next = {};
    ...
    function extractFunctionComment(functionName) {
        ...
        var comments = before.split("/**");
        var comment = "/**" + comments.pop();
        if (comment.match(/\r\n\r\n/)) {
            return;
        }
        if (comment.match(/function \w+\(/)) {
            return;
        }
