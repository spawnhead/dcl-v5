...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    /* Prevent global CSS from inflating cell line-height */
...
<body>
  <div class="toolbar">
    ...
    <button type="button" class="btn btn-ghost" id="clearFilter">Сбросить фильтр</button>
  ...
  (function() {
    ...
    function parseDate(value) {
      ...
      if (/^\d{2}\.\d{2}\.\d{4}$/.test(v)) {
        m = v.split('.');
        return new Date(parseInt(m[2], 10), parseInt(m[1], 10) - 1, parseInt(m[0], 10));
      }
      if (/^\d{4}-\d{2}-\d{2}$/.test(v)) {
        m = v.split('-');
        return new Date(parseInt(m[0], 10), parseInt(m[1], 10) - 1, parseInt(m[2], 10));
      }
      return null;
    }
    ...
    var dateFilterParams = {
      comparator: function(filterLocalDateAtMidnight, cellValue) {
        var d = parseDate(cellValue);
        if (!d) return -1;
        if (d < filterLocalDateAtMidnight) return -1;
        if (d > filterLocalDateAtMidnight) return 1;
        return 0;
      },
    ...
    var columnDefs = [
      ...
      { field: 'con_date_formatted', headerName: '<bean:message key="Margin.con_date"/>', minWidth: 110, filter: 'agDateColumnFilter', filterParams: dateFilterParams },
      { field: 'spc_number_formatted', headerName: '<bean:message key="Margin.spc_number"/>', minWidth: 140 },
      { field: 'spc_date_formatted', headerName: '<bean:message key="Margin.spc_date"/>', minWidth: 130, filter: 'agDateColumnFilter', filterParams: dateFilterParams },
      ...
      { field: 'shp_date_show', headerName: '<bean:message key="Margin.shp_date"/>', minWidth: 120, filter: 'agDateColumnFilter', filterParams: dateFilterParams },
      { field: 'pay_date_show', headerName: '<bean:message key="Margin.pay_date"/>', minWidth: 120, filter: 'agDateColumnFilter', filterParams: dateFilterParams },
    ...
    if (exportBtn) exportBtn.onclick = function() {
      if (!gridApi) return;
      gridApi.exportDataAsCsv({ fileName: 'margin_export.csv' });
